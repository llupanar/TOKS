...
fn frame_for_print (rframe: String,fcs_index: usize)-> String{
 ...
 pub fn decode_hamming(rframe:String)->(String,String,i32){
	 ...
	 let mut data = rframe[20..].to_string();
	 match data.len(){
	...
	fcs = data[data.len() - fcs_size..].to_string();
	data = data[..data.len()-fcs_size].to_string();	
	...
	if check_fcs==fcs { 
		 ...
		 if check_fcs[check_fcs.len()-1 as usize..]==fcs[fcs.len()-1 as usize..]{
